...
import { Reference, type AssignOptions, type Collection, type EntityFactory, type EntityIdentifier, type EntityLoaderOptions, type EntityRepository, type IdentifiedReference } from './entity';
...
export interface IWrappedEntity<T, PK extends keyof T | unknown = PrimaryProperty<T>, P extends string = string> {
...
export type EntityDataProp<T> = T extends Date ? string | Date : T extends Scalar ? T : T extends Reference<infer U> ? EntityDataNested<U> : T extends Collection<infer U, any> ? U | U[] | EntityDataNested<U> | EntityDataNested<U>[] : T extends readonly (infer U)[] ? U | U[] | EntityDataNested<U> | EntityDataNested<U>[] : EntityDataNested<T>;
...
type IsOptional<T, K extends keyof T> = T[K] extends Collection<any, any> ? true : T[K] extends Function ? true : K extends symbol ? true : K extends ProbablyOptionalProps<T> ? true : false;
...
export type RequiredEntityData<T> = EntityData<T> & {
...
export type EntityDTOProp<T> = T extends Scalar ? T : T extends LoadedReference<infer U> ? EntityDTO<U> : T extends Reference<infer U> ? Primary<U> : T extends {
    getItems(check?: boolean): infer U;
} ? (U extends readonly (infer V)[] ? EntityDTO<V>[] : EntityDTO<U>) : T extends {
    $: infer U;
} ? (U extends readonly (infer V)[] ? EntityDTO<V>[] : EntityDTO<U>) : T extends readonly (infer U)[] ? (T extends readonly [infer U, ...infer V] ? T : U[]) : T extends Relation<T> ? EntityDTO<T> : T;
...
export type AutoPath<O, P extends string, E extends string = never, D extends Prev[number] = 5> = [
    D
] extends [never] ? string : P extends any ? (P & `${string}.` extends never ? P : P & `${string}.`) extends infer Q ? Q extends `${infer A}.${infer B}` ? A extends StringKeys<O, E> ? `${A}.${AutoPath<Defined<GetStringKey<O, A, E>>, B, E, Prev[D]>}` : never : Q extends StringKeys<O, E> ? (Defined<GetStringKey<O, Q, E>> extends unknown ? Exclude<P, `${string}.`> : never) | (StringKeys<Defined<GetStringKey<O, Q, E>>, E> extends never ? never : `${Q}.`) : StringKeys<O, E> : never : never;
...
type LoadedLoadable<T, E extends object> = T extends Collection<any, any> ? T & LoadedCollection<E> : (T extends Reference<any> ? T & LoadedReference<E> : T & E);
