...
function createContextualWorkflowRunner({ workflowId, defaultResult, dataPreparation, options, container, }) {
    ...
    const originalExecution = async (method, { throwOnError, resultFrom, isCancel = false }, ...args) => {
    ...
    const newCancel = async ({ transaction, transactionId, context: outerContext, throwOnError, events, } = {
        throwOnError: true,
    }) => {
        throwOnError ?? (throwOnError = true);
        const context = {
