...
class LocalWorkflow {
    ...
    async getRunningTransaction(uniqueTransactionId, context) {
        this.medusaContext = context;
        const { handler, orchestrator } = this.workflow;
        const transaction = await orchestrator.retrieveExistingTransaction(uniqueTransactionId, handler(this.container, context));
        return transaction;
    }
    ...
    async registerStepSuccess(idempotencyKey, response, context, subscribe) {
        ...
        const transaction = await orchestrator.registerStepSuccess(idempotencyKey, handler(this.container, context), undefined, response);
        cleanUpEventListeners();
        return transaction;
    }
    async registerStepFailure(idempotencyKey, error, context, subscribe) {
        ...
        const transaction = await orchestrator.registerStepFailure(idempotencyKey, error, handler(this.container, context));
        cleanUpEventListeners();
        return transaction;
    }
    setOptions(options) {
