...
export interface ICsvValidator<TCsvLine, TBuiltLine> {
     ...
     */
    validate: (value: TBuiltLine, context: CsvParserContext<TCsvLine>) => Promise<boolean | never>;
}
 ...
 */
export declare abstract class AbstractCsvValidator<TCsvLine, TBuiltLine> implements ICsvValidator<TCsvLine, TBuiltLine> {
...
export type CsvSchemaColumn<TCsvLine, TBuiltLine, NameAsOptional = false> = (NameAsOptional extends false ? {
    name: string;
} : {
    name?: string;
}) & {
...
export type ColumnTransformer<TCsvLine> = (value: string, context: CsvParserContext<TCsvLine>) => unknown;
export type ColumnReducer<TCsvLine = unknown, TBuiltLine = unknown> = (builtLine: TBuiltLine, key: string, value: string, context: CsvParserContext<TCsvLine>) => TBuiltLine;
