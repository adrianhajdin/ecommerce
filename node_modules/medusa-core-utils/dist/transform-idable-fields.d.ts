type ComputePropertyNames<T, TKey extends keyof T & string, TFields extends (keyof T | string)[] = (keyof T | string)[]> = TKey extends TFields[number] ? T[TKey] extends string ? `${TKey}_id` : TKey : TKey;
/**
 ...
 * ```
 */
export declare const transformIdableFields: <T extends object = Record<string, unknown>, TFields extends (string | keyof T)[] = (string | keyof T)[], TOutput = { [P in ComputePropertyNames<T, keyof T & string, TFields>]: P extends keyof T ? T[P] : string; }>(obj: T, fields: TFields) => TOutput;
export {};
