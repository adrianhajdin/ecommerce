...
export declare type UmzugOptions<Ctx extends {} = Record<string, unknown>> = {
...
export declare type MigrationParams<T> = {
    name: string;
    path?: string;
    context: T;
};
/** A callable function for applying or reverting a migration  */
export declare type MigrationFn<T = unknown> = (params: MigrationParams<T>) => Promise<unknown>;
/**
 ...
 */
export declare type RunnableMigration<T> = {
...
export declare type GlobInputMigrations<T> = {
    /**
 ...
 */
export declare type InputMigrations<T> = GlobInputMigrations<T> | Array<RunnableMigration<T>> | ((context: T) => Promisable<InputMigrations<T>>);
/** A function which takes a migration name, path and context, and returns an object with `up` and `down` functions. */
export declare type Resolver<T> = (params: MigrationParams<T>) => RunnableMigration<T>;
...
export declare type RerunBehavior = keyof typeof RerunBehavior;
export declare type MigrateUpOptions = MergeExclusive<{
...
export declare type MigrateDownOptions = MergeExclusive<{
...
export declare type UmzugEvents<Ctx> = {
